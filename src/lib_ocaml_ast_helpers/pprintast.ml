...
let varify_type_constructors var_names t =
  let check_variable vl loc v =
    if List.mem v vl
    then Location.raise_errorf ~loc "variable in scope syntax error: %s" v in
  ...
  let rec loop t =
    let desc =
      ...
      | Ptyp_poly (string_lst, core_type) ->
        List.iter (fun v -> check_variable var_names t.ptyp_loc v.txt) string_lst;
        Ptyp_poly (string_lst, loop core_type)
      | Ptyp_package (longident, lst) ->
        Ptyp_package (longident, List.map (fun (n, typ) -> n, loop typ) lst)
      | Ptyp_extension (s, arg) -> Ptyp_extension (s, arg) in
  ...
  and loop_object_field field =
    let pof_desc =
      ...
      | Oinherit t -> Oinherit (loop t) in
    { field with pof_desc } in
  loop t
...
(* determines if the string is an infix string. checks backwards, first allowing a
   renaming postfix ("_102") which may have resulted from Pexp -> Texp -> Pexp
   translation, then checking if all the characters in the beginning of the string
   are valid infix characters. *)
...
let needs_parens txt =
  ...
  is_infix fix || is_mixfix fix || is_kwdop fix || first_is_in prefix_symbols txt
...
(* let reset_semi ctxt = { ctxt with semi=false } let reset_ifthenelse ctxt = {
   ctxt with ifthenelse=false } let reset_pipe ctxt = { ctxt with pipe=false } *)
  ...
  let aux f = function
    ...
    | xs ->
      let rec loop f = function
        ...
        | _ -> assert false in
      pp f first; loop f xs; pp f last in
  aux f xs
...
let paren :
  ...
  if b then (pp f "("; pp f first; fu f x; pp f last; pp f ")") else fu f x
...
let tyvar ppf s =
  ...
  then
    (* without the space, this would be parsed as a character literal *)
...
and core_type ctxt f x =
  ...
  then
    ...
    | Ptyp_alias (ct, s) -> pp f "@[<2>%a@;as@;%a@]" (core_type1 ctxt) ct tyvar s
  ...
  then core_type ctxt f x
    ...
    | Ptyp_object (l, o) ->
      let core_field_type f x =
        ...
        | Oinherit ct -> pp f "@[<hov2>%a@ @]" (core_type ctxt) ct in
      let field_var f = function
        ...
        | Asttypes.Open -> (
          ...
          | [] -> pp f ".."
          | _ -> pp f " ;..") in
      pp f "@[<hov2><@ %a%a@ > @]" (list core_field_type ~sep:";") l field_var o
      (* Cf #7200 *)
    | Ptyp_class (li, l) ->
      (*FIXME*)
      pp f "@[<hov2>%a#%a@]"
...
and pattern1 ctxt (f : Format.formatter) (x : pattern) : unit =
  let rec pattern_list_helper f = function
    ...
    | p -> pattern1 ctxt f p in
  if x.ppat_attributes <> []
  then pattern ctxt f x
  else (
    match x.ppat_desc with
    | Ppat_variant (l, Some p) -> pp f "@[<2>`%s@;%a@]" l (simple_pattern ctxt) p
    ...
    | Ppat_construct (({ txt; _ } as li), po) ->
      ...
      then pp f "%a" pattern_list_helper x
        ...
        | Some (vl, x) ->
          pp f "%a@ (type %a)@;%a" longident_loc li (list ~sep:"@ " string_loc) vl
  ...
  then pattern ctxt f x
    ...
    | Ppat_record (l, closed) -> (
      let longident_x_pattern f (li, p) =
        ...
        | ( { txt = Lident s; _ },
          ...
          when s = txt -> pp f "@[<2>%a@]" longident_loc li
        | _ -> pp f "@[<2>%a@;=@;%a@]" longident_loc li (pattern1 ctxt) p in
      match closed with
    ...
    | Ppat_open (lid, p) ->
      let with_paren =
        ...
        | _ -> true in
      pp f "@[<2>%a.%a @]" longident_loc lid (paren with_paren @@ pattern1 ctxt) p
...
and sugar_expr ctxt f e =
  ...
  then false
    ...
    | Pexp_apply
      ...
      let print_indexop
          a path_prefix assign left sep right print_index indices rem_args
        =
        ...
        | false, [] ->
          pp f "@[%a%a%s%a%s@]" (simple_expr ctxt) a print_path path_prefix left
            (list ~sep print_index) indices right;
          true
        | true, [ v ] ->
      ...
      | (Lident s | Ldot (_, s)), a :: i :: rest when first_is '.' s ->
        (* extract operator: assignment operators end with [right_bracket ^ "<-"],
...
and expression ctxt f x =
  ...
  then
    ...
    | Pexp_sequence _
    | Pexp_newtype _
      when ctxt.pipe || ctxt.semi -> paren true (expression reset_ctxt) f x
    ...
    | Pexp_letexception _
    | Pexp_letop _
      when ctxt.semi -> paren true (expression reset_ctxt) f x
    | Pexp_fun (l, e0, p, e) ->
      pp f "@[<2>fun@;%a->@;%a@]" (label_exp ctxt) (l, e0, p) (expression ctxt) e
    | Pexp_newtype (lid, e) ->
    ...
    | Pexp_try (e, l) ->
      pp f "@[<0>@[<hv2>try@ %a@]@ @[<0>with%a@]@]"
        ...
        e (case_list ctxt) l
    | Pexp_let (rf, l, e) ->
      (* pp f "@[<2>let %a%a in@;<1 -2>%a@]" (*no indentation here, a new
         line*) *)
      (* rec_flag rf *)
        ...
        | `Prefix s -> (
          let s =
            if List.mem s [ "~+"; "~-"; "~+."; "~-." ]
               &&
               match l with
               (* See #7200: avoid turning (~- 1) into (- 1) which is parsed as an
                  int literal *)
    ...
    (* Pexp_poly: impossible but we should print it anyway, rather than assert
       false *)
    | Pexp_poly (e, None) -> pp f "@[<hov2>!poly!@ %a@]" (simple_expr ctxt) e
    | Pexp_poly (e, Some ct) ->
      pp f "@[<hov2>(!poly!@ %a@ : %a)@]" (simple_expr ctxt) e (core_type ctxt) ct
    | Pexp_open (o, e) ->
      pp f "@[<2>let open%s %a in@;%a@]" (override o.popen_override)
...
and simple_expr ctxt f x =
  ...
  then expression ctxt f x
    ...
    | Pexp_tuple l -> pp f "@[<hov2>(%a)@]" (list (simple_expr ctxt) ~sep:",@;") l
    ...
    | Pexp_record (l, eo) ->
      let longident_x_expression f (li, e) =
        ...
        | { pexp_desc = Pexp_ident { txt; _ }; pexp_attributes = []; _ }
          when li.txt = txt -> pp f "@[<hov2>%a@]" longident_loc li
...
and value_description ctxt f x =
  (* note: value_description has an attribute field, but they're already printed
     by the callers this method *)
  pp f "@[<hov2>%a%a@]" (core_type ctxt) x.pval_type
    (fun f x ->
      if x.pval_prim <> [] then pp f "@ =@ %a" (list constant_string) x.pval_prim)
    x
...
and class_type_field ctxt f x =
  ...
  | Pctf_constraint (ct1, ct2) ->
    pp f "@[<2>constraint@ %a@ =@ %a@]%a" (core_type ctxt) ct1 (core_type ctxt)
      ct2 (item_attributes ctxt) x.pctf_attributes
...
and class_type ctxt f x =
  ...
  | Pcty_extension e ->
    ...
    attributes ctxt f x.pcty_attributes
  | Pcty_open (o, e) ->
    pp f "@[<2>let open%s %a in@;%a@]" (override o.popen_override) longident_loc
      o.popen_expr (class_type ctxt) e
...
and class_type_declaration_list ctxt f l =
  let class_type_declaration kwd f x =
    ...
    pp f "@[<2>%s %a%a%s@ =@ %a@]%a" kwd virtual_flag x.pci_virt
      ...
      (item_attributes ctxt) x.pci_attributes in
  match l with
  | [] -> ()
...
and class_field ctxt f x =
  ...
  | Pcf_val (s, mf, Cfk_virtual ct) ->
    pp f "@[<2>val virtual %a%s :@ %a@]%a" mutable_flag mf s.txt (core_type ctxt)
      ct (item_attributes ctxt) x.pcf_attributes
  | Pcf_method (s, pf, Cfk_concrete (ovf, e)) ->
    ...
    pp f "@[<2>method%s %a%a@]%a" (override ovf) private_flag pf
      ...
      | { pexp_desc = Pexp_poly (e, None); pexp_attributes = []; _ } -> bind e
      | _ -> bind e)
      e (item_attributes ctxt) x.pcf_attributes
  | Pcf_constraint (ct1, ct2) ->
    pp f "@[<2>constraint %a =@;%a@]%a" (core_type ctxt) ct1 (core_type ctxt) ct2
...
and class_expr ctxt f x =
  ...
  then
    ...
    | Pcl_open (o, e) ->
      pp f "@[<2>let open%s %a in@;%a@]" (override o.popen_override) longident_loc
        o.popen_expr (class_expr ctxt) e)

and module_type ctxt f x =
  if x.pmty_attributes <> []
  then
    ...
    | Pmty_functor (Named (s, mt1), mt2) -> (
      match s.txt with
      | None ->
        pp f "@[<hov2>%a@ ->@ %a@]" (module_type1 ctxt) mt1 (module_type ctxt) mt2
      | Some name ->
        pp f "@[<hov2>functor@ (%s@ :@ %a)@ ->@ %a@]" name (module_type ctxt) mt1
...
and signature_item ctxt f x : unit =
  ...
  | Psig_typesubst l ->
    (* Psig_typesubst is never recursive, but we specify [Recursive] here to avoid
       printing a [nonrec] flag, which would be rejected by the parser. *)
  ...
  | Psig_class l -> (
    let class_description
        kwd f ({ pci_params = ls; pci_name = { txt; _ }; _ } as x)
      =
      pp f "@[<2>%s %a%a%s@;:@;%a@]%a" kwd virtual_flag x.pci_virt
        (class_params_def ctxt) ls txt (class_type ctxt) x.pci_expr
        (item_attributes ctxt) x.pci_attributes in
    ...
    | x :: xs ->
      pp f "@[<v>%a@,%a@]" (class_description "class") x
  ...
  | Psig_module pmd ->
    pp f "@[<hov>module@ %s@ :@ %a@]%a"
      (Option.value pmd.pmd_name.txt ~default:"_")
      (module_type ctxt) pmd.pmd_type (item_attributes ctxt) pmd.pmd_attributes
  | Psig_modsubst pms ->
  ...
  | Psig_extension (e, a) -> item_extension ctxt f e; item_attributes ctxt f a

and module_expr ctxt f x =
  if x.pmod_attributes <> []
  then
...
and payload ctxt f = function
  | PStr [ { pstr_desc = Pstr_eval (e, attrs) } ] ->
    pp f "PStr1:@[<2>%a@]%a" (expression ctxt) e (item_attributes ctxt) attrs
  | PStr x -> pp f "Pstr2:"; structure ctxt f x
  | PTyp x -> pp f "PTyp:@ "; core_type ctxt f x
  | PSig x -> pp f "PSig::@ "; signature ctxt f x
  | PPat (x, None) -> pp f "PPat:?@ "; pattern ctxt f x
  | PPat (x, Some e) ->
    pp f "PPat:?@ "; pattern ctxt f x; pp f " when "; expression ctxt f e
...
and bindings ctxt f (rf, l) =
  let binding kwd rf f x =
    pp f "@[<2>%s %a%a@]%a" kwd rec_flag rf (binding ctxt) x
      (item_attributes ctxt) x.pvb_attributes in
  match l with
  | [] -> ()
...
and binding_op ctxt f x =
  ...
  | ( { ppat_desc = Ppat_var { txt = pvar; _ }; ppat_attributes = []; _ },
    ...
    when pvar = evar -> pp f "@[<2>%s %s@]" x.pbop_op.txt evar
  | pat, exp ->
    pp f "@[<2>%s %a@;=@;%a@]" x.pbop_op.txt (pattern ctxt) pat (expression ctxt)
      exp
...
and structure_item ctxt f x =
  ...
  | Pstr_module x ->
    let rec module_helper = function
      | { pmod_desc = Pmod_functor (arg_opt, me'); pmod_attributes = [] } ->
        ...
        | Unit -> pp f "()"
        | Named (s, mt) ->
          pp f "(%s:%a)" (Option.value s.txt ~default:"_") (module_type ctxt) mt);
        module_helper me'
      | me -> me in
  ...
  | Pstr_open od ->
    pp f "@[<2>open%s@;%a@]%a"
      (override od.popen_override)
      (module_expr ctxt) od.popen_expr (item_attributes ctxt) od.popen_attributes
  ...
  | Pstr_class l -> (
    ...
    | x :: xs ->
      pp f "@[<v>%a@,%a@]" (class_declaration "class") x
  ...
  | Pstr_recmodule decls -> (
    ...
    | pmb :: l2 ->
      pp f "@[<hv>@[<hov2>module@ rec@ %s@ =@ %a@]%a@ %a@]"
        (Option.value pmb.pmb_name.txt ~default:"_")
        (module_expr ctxt) pmb.pmb_expr (item_attributes ctxt) pmb.pmb_attributes
        (fun f l2 -> List.iter (aux f) l2)
        l2
    | _ -> assert false)
  | Pstr_attribute a -> floating_attribute ctxt f a
  | Pstr_extension (e, a) -> item_extension ctxt f e; item_attributes ctxt f a
...
and type_def_list ctxt f (rf, exported, l) =
  let type_decl kwd rf f x =
    let eq =
      if x.ptype_kind = Ptype_abstract && x.ptype_manifest = None
      then ""
      else if exported
      then " ="
      else " :=" in
    pp f "@[<2>%s %a%a%s%s%a@]%a" kwd nonrec_flag rf (type_params ctxt)
      ...
      (item_attributes ctxt) x.ptype_attributes in
  match l with
  | [] -> assert false
...
and record_declaration ctxt f lbls =
  let type_record_field f pld =
    pp f "@[<2>%a%s:@;%a@;%a@]" mutable_flag pld.pld_mutable pld.pld_name.txt
      (core_type ctxt) pld.pld_type (attributes ctxt) pld.pld_attributes in
  pp f "{@\n%a}" (list type_record_field ~sep:";@\n") lbls

and type_declaration ctxt f x =
  (* type_declaration has an attribute field, but it's been printed by the caller
     of this method *)
  let priv f =
    match x.ptype_private with
    | Public -> ()
    | Private -> pp f "@;private" in
  let manifest f =
    match x.ptype_manifest with
    | None -> ()
    | Some y ->
      if x.ptype_kind = Ptype_abstract
      then pp f "%t@;%a" priv (core_type ctxt) y
      else pp f "@;%a" (core_type ctxt) y in
...
and constructor_declaration ctxt f (name, vars, args, res, attrs) =
  ...
  | None ->
    pp f "%s%a@;%a" name
      (fun f -> function
      | Pcstr_tuple [] -> ()
      | Pcstr_tuple l -> pp f "@;of@;%a" (list (core_type1 ctxt) ~sep:"@;*@;") l
...
and extension_constructor ctxt f x =
  ...
  | Pext_decl (v, l, r) ->
    constructor_declaration ctxt f (x.pext_name.txt, v, l, r, x.pext_attributes)
  | Pext_rebind li ->
...
let string_of_expression x =
  ...
  expression f x; flush_str_formatter ()

let string_of_structure x =
  ignore (flush_str_formatter ());
  let f = str_formatter in
  structure reset_ctxt f x; flush_str_formatter ()

let top_phrase f x =
  pp_print_newline f (); toplevel_phrase f x; pp f ";;"; pp_print_newline f ()
